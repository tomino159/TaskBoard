...
const fanCards = [
  { label: "Slúchadlá", price: "49,90 €", rotate: -16, dx: -170 },
  { label: "Hodinky", price: "89,00 €", rotate: 0, dx: 0 },
  { label: "Nabíjačka", price: "24,50 €", rotate: 16, dx: 170 },
];

export const DashboardScene1: React.FC = () => {
  const frame = useCurrentFrame();
  const count = Math.round(useCountUp(0, 47, 34, 22));
  const float = Math.sin(frame * 0.09) * 6;
  const bannerGlow = 0.4 + 0.25 * Math.sin(frame * 0.15);

  const fanOutOpacity = interpolate(frame, [26, 34], [1, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  const mockupOpacity = interpolate(frame, [24, 32], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
    ...
    >
      <CameraPunchIn from={1} to={1.05}>
        ...
        >
          ...
          <div style={{ position: "relative", width: 860, height: 480 }}>
            <div
              style={{
                position: "absolute",
                inset: 0,
            ...
            >
              <BrowserMockupTilt startFrame={28}>
                <div
                  style={{
                    display: "flex",
                    height: 480,
                ...
                <CursorDot x={640} y={220} startFrame={46} clickFrame={58} />
              </BrowserMockupTilt>
            </div>
          </div>
        </AbsoluteFill>
